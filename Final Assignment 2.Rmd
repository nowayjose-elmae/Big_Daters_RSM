...
Set.Seed(123). This given that the parameters were adapted instead of
the seed. This ensure that the data can be replicated and is consistent
across "random" simulations.

It is assumed that each combination of four features is a unique agent.
Given that the nature of these features is unkown, it is possible that
...
same unique features depending on its nature. In the analysis, both
would fall under Agent 106. This decision was made given the uncertainty
of these features, and hence understands that more specific features
lead to a more realistic unique agent identification.

# VI Data Aggregation and Heterogeneity

## DH1.1 Heterogeniety and Aggregation

As per how it was addressed at the Overview of Methodology, the analysis
will follow to stages. One in which it is assumed that each datapoint is
an individual user (Un-aggregated) and another in which the interaction
...
providing a multiplicity amount of unique agent. Each agent has a unique
combination of these four unique features. Hence, the reproduction of
all possible agents are given by the following code:
...
multiple times with the add, yet they never click. Other agents click
...
clicked at least once) and the "Passive" agents (Those who not clicked).
The analysis will be focused towards the "Active Agents". It will
suggest an item and position to users who the algorithm has feedback to
learn from. These is identified with the code below, where the code
filters out those agents who only access, yet never click the adds, the
...
with the page in a meaningful way. This are the potential costumers that
the MAB should target in a way that it suggests the right add and
position for them.
...
set is summarised with the table below:
...
In this part, when implementing the epsilon-greedy algorithm to choose
the best performing arm(item_id) from our dataset. The arm that is
chosen is 11, proving that histogram 2 ("Click rate per item") alligns
...
### E1.2 Simulation Greedy: Simulates perfomance of an epsilon-greedy policy
...
The parameters for Epsilon-Greedy analysis are: Fisrtly, n_before_sim
indicates the number of observations before the starting of
...
The following code rerpresentes a separate histogram with its confidence
...
test was computed. This shows if there is any significant differences
...
### E2.2 Simulation Greedy: Simulates perfomance of an epsilon-greedy policy on aggregated data
...
The parameter from E1.3 were held constant.
...
Finally, the graph provides the analsis of the performance of the
...
Based on the summary of mean_click per arms, it can be concluded that
there is much more failures than success since the mean_click values are
...
(1,90) and (3,90) for this dataset. We will use this for the further
analysis. The result of the other pairs can be found in the appendix.
...
Algorithm analysis different policies and assess which one performs
better over time.
...
Additionally, the UCB classifier varies in effectiveness along the
...
However, there can be the case in which the same user interacts with the
website multiple times.

In reality, each agent (user) has common characteristics or unique user
features which models their behavior. The dataset provided from ZOZO
...
group the interaction by user, meaning aggregating the dataset by
...
position is better for which Active Customer. This is seen with the
table below:
...
combination. Now each combiantion of item_id and position is regarded as
a new arm. Hence, the program is ran with a 30-armed bandit (3
...
# Step 7: Calculate variance of average rewards across batch sizes
...
Sampling seems to be the weakest method of the chosen three. This only
within the context of batching. Batching is a determining element if the
...
be furthered compared which are the Epsilon-Greedy, and the Upper
Confidence Bound.

Epsilon-Greedy surpasses the UCB algorithm for average total rewards in
every single batching. However, it has a more siginificant variance
...
degreee of *curiosity*. For example, UCB Agent (C=0.5) has an Epsiloner
...
methods, expect for the first two agents where UCB outperforms Epsilon,
...
analysis is to explore the differences accross three Machine Learning
...
performing Epsilon, measure by the average cumulative rewards accross
...
intial (Alpha = 2) and (Beta = 70). This given the low rate of clicks
...
The three methods aggree that the most common CTR maximizing position in
2. Two of the methods agree that **Position 2** and **Item 15**
represents the CTR maximizing combination with regards to their best
perfoming parameters and the given aggregated dataset in which the 8
potential active costumers clicked. Hence, this can be assumed to be the
...
it can observe and indentify better which arm is to be exploited.

When choosing between the two methods, if the company expects customers
with low levels of exploration then UCB inheretedly outperforms the
...
The research purpose highlighted the analysis of the results and its
...
has concluded that the best combination to maximize the CTR, aggreed by
...
(D1.2) , and **Position 2** is the most common amogst the three (D1.3).
...
an optimal agents, non-active users (Agents who didn't click at least
once) can be suggested the optimal combination for the agent with the
...
logic of progression. The analysis was primarily done without blackbox
...
it was being computed in another device. Lastly, given the limited
...
This is something that must be taken into account when assesing the
