...
```{r}
set.seed(0)
max_t <- min(df_TS_max_t$max_t) #Thw lowest value in the max_t for each dataset
...
# Load the dataset (assuming dfZozo_20 contains 'item_id' and 'click' columns)
#dfZozo_20 <- read.csv("zozo_noContext_20items_tiny.csv")
...
After comparing the result with various different pairs of alphas and betas, the best pair of alphas and betas were chosen to be (1,60) for this dataset. We will use this for the further analysis. The result of the other pairs can be found in the appendix.
...
The results show that the maximum number of rounds across 20 simulations varied between 435 and 538. While most simulations fall within a narrow range, this variation suggests differences in exploration patterns, where some simulations explored more arms or took more actions before reaching the end of the dataset.
...
# TS best item, position
```{r, TS best item, position}

# Create bandit
bandit_TS_item <- OfflineReplayEvaluatorBandit$new(formula = click ~ item_id,
                                                   data = df_filtered_agents,
                                                   randomize = FALSE)

bandit_TS_position <- OfflineReplayEvaluatorBandit$new(formula = click ~ position,
                                                   data = df_filtered_agents,
                                                   randomize = FALSE)

bandit_TS_combination <- OfflineReplayEvaluatorBandit$new(formula = click ~ combination_index,
                                                   data = df_filtered_agents,
                                                   randomize = FALSE)
# Define the Thompson Sampling policy
TS <- ThompsonSamplingPolicy$new(alpha = 1, beta = 90)

# Create the agent for 80 arms
agent_TS_item <- Agent$new(TS, bandit_TS_item)
agent_TS_position <- Agent$new(TS, bandit_TS_position)
agent_TS_combination <- Agent$new(TS, bandit_TS_combination)


# Run the simulator for Thompson Sampling
simulator_TS_item <- Simulator$new(agent_TS_item, 
                              horizon = size_sim, 
                              simulations = n_sim)
history_TS_item<- simulator_TS_item$run()

simulator_TS_position <- Simulator$new(agent_TS_position, 
                              horizon = size_sim, 
                              simulations = n_sim)
history_TS_position<- simulator_TS_position$run()

simulator_TS_combination <- Simulator$new(agent_TS_combination, 
                              horizon = size_sim, 
                              simulations = n_sim)
history_TS_combination <- simulator_TS_combination$run()


# Gather results for Thompson Sampling
df_history_item <- history_TS_item$data %>%
  select(t, sim, item = choice, reward, agent) 

df_history_position <- history_TS_position$data %>%
  select(t, sim, position = choice, reward, agent) 

df_history_combination <- history_TS_combination$data %>%
  select(t, sim, combination=choice, reward, agent) 

# Step 8: Calculate cumulative rewards for each simulation
df_cumulative_item <- df_history_item %>%
  group_by(sim) %>%
  mutate(cumulative_reward = cumsum(reward)) %>%
  ungroup()

df_cumulative_position <- df_history_position %>%
  group_by(sim) %>%
  mutate(cumulative_reward = cumsum(reward)) %>%
  ungroup()

df_cumulative_combination <- df_history_combination %>%
  group_by(sim) %>%
  mutate(cumulative_reward = cumsum(reward)) %>%
  ungroup()


# Step 9: Calculate average cumulative rewards by position for item 9
avg_reward_by_item <- df_cumulative_item%>%
  group_by(item) %>%
  summarise(avg_cumulative_reward = mean(cumulative_reward, na.rm = TRUE),
            .groups = 'drop') %>%
  arrange(desc(avg_cumulative_reward))

avg_reward_by_position <- df_cumulative_position%>%
  group_by(position) %>%
  summarise(avg_cumulative_reward = mean(cumulative_reward, na.rm = TRUE),
            .groups = 'drop') %>%
  arrange(desc(avg_cumulative_reward))

avg_reward_by_combination <- df_cumulative_combination%>%
...
# Step 10: Get the position with the highest average cumulative reward for item 9
highest_avg_item <- avg_reward_by_item %>% 
  slice(1)

highest_avg_position <- avg_reward_by_position %>% 
  slice(1)
highest_avg_combo<- avg_reward_by_combination %>% 
  slice(1)

highest_avg_combination<- df_filtered_agents %>%
  filter(combination_index == highest_avg_combo$combination) %>%
  select(combinations) %>%
  slice(1)

# Output results
cat("\nHighest Average Cumulative Reward by item:\n")
print(highest_avg_item)
cat("\nHighest Average Cumulative Reward by position:\n")
print(highest_avg_position)
cat("\nHighest Average Cumulative Reward by combination:\n")
print(highest_avg_combination) 
...
This code implements Thompson Sampling with a batch size of 1, using alpha=1 and beta=60, which were found to be the best-performing parameters from previous analysis. The algorithm is designed to identify the best-performing arm in a multi-armed bandit problem based on cumulative successes over 10 simulations, each with 4442 rounds. For each round, the algorithm samples from a Beta distribution for each arm, selects the arm with the highest sampled value, and simulates a reward based on the true CTR of the chosen arm. Successes and failures for each arm are updated after every round, and the arm with the most cumulative successes across simulations is identified as the best.
...
df_history_agg <- df_TS_zozo_20 %>%
  ...
  summarise(
    ...
    se_cumulative_reward = sd(cumulative_reward, na.rm = TRUE) / sqrt(n()), 
    .groups = 'drop'
  ) %>%
  mutate(
    cumulative_reward_lower_CI = avg_cumulative_reward - 1.96 * se_cumulative_reward,
