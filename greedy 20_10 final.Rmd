...
This chunk loads the full dataset with 20 items (arms) and create an
index column that we are using in our analysis later.
...
We perform a summary statistics in order to get a better overview of the
dataset.
...
filter and merge the agents and finally, in Part 3 we create the final
...
### 6. Batch size analysis on Greedy (aggregated data)

```{r}
# Step 1: Initialize matrices for Epsilon-Greedy algorithm
n_users <- length(unique(agents_data$Agent))  # Number of users
n_items <- length(unique(item_position_combinations$item_id))  # Number of items (or combinations of items/positions)

# Initialize reward and trial matrices for Epsilon-Greedy algorithm
rewards_eps_greedy <- matrix(0, nrow = n_users, ncol = n_items)  # Rewards for Epsilon-Greedy
trials_eps_greedy <- matrix(0, nrow = n_users, ncol = n_items)  # Trials for Epsilon-Greedy

# Step 2: Define batch sizes and simulation parameters
batch_sizes <- c(1, 10, 50, 100, 500)  # Batch sizes to test
n_rounds <- 1000  # Total rounds for simulation

# Create a dataframe to store the results
results <- data.frame()

epsilon <- 0.4  # Epsilon value for epsilon-greedy algorithm

# Step 3: Run simulations for the Epsilon-Greedy algorithm
for (batch_size in batch_sizes) {
  eps_greedy_total_rewards <- 0
  
  for (t in seq(1, n_rounds, by = batch_size)) {
    # Simulate user interactions for each batch
    for (b in 1:batch_size) {
      for (user in 1:n_users) {
        
        ### Epsilon-Greedy Algorithm (ε = 0.4) ###
        # Explore or exploit based on epsilon
        if (runif(1) < epsilon) {
          chosen_eps_greedy <- sample(1:n_items, 1)  # Exploration: random item
        } else {
          # Exploitation: choose item-position with the highest score
          eps_greedy_scores <- rewards_eps_greedy[user, ] / (trials_eps_greedy[user, ] + 1e-8)
          chosen_eps_greedy <- which.max(eps_greedy_scores)
        }
        
        # Retrieve item and position for the epsilon-greedy choice
        chosen_item_eps <- item_position_combinations$item_id[chosen_eps_greedy]
        chosen_position_eps <- item_position_combinations$position[chosen_eps_greedy]
        
        # Simulate clicks for the chosen item and position
        click_data_eps <- agents_data[agents_data$Agent == unique(agents_data$Agent)[user] & agents_data$item_id == chosen_item_eps & agents_data$position == chosen_position_eps, "click"]
        
        # Update rewards and trials if clicks are present
        if (length(click_data_eps) > 0) {
          reward_eps <- sum(click_data_eps)
          rewards_eps_greedy[user, chosen_eps_greedy] <- rewards_eps_greedy[user, chosen_eps_greedy] + reward_eps
          trials_eps_greedy[user, chosen_eps_greedy] <- trials_eps_greedy[user, chosen_eps_greedy] + 1
          eps_greedy_total_rewards <- eps_greedy_total_rewards + reward_eps
        }
      }
    }
    
    # Calculate average reward for the Epsilon-Greedy algorithm over the batch
    eps_greedy_avg_reward <- eps_greedy_total_rewards / (batch_size * n_users)
    
    # Store the results for Epsilon-Greedy algorithm
    results <- rbind(results, data.frame(
      batch_size = batch_size,
      avg_reward = eps_greedy_avg_reward,
      algorithm = paste0("Epsilon-Greedy (ε = ", epsilon, ")")
    ))
  }
}

# Step 4: Identify the batch size with the highest average reward
best_batch <- results[which.max(results$avg_reward), "batch_size"]
best_reward <- max(results$avg_reward)

# Step 1: Create the histogram without black labels
ggplot(data = results, aes(x = factor(batch_size), y = avg_reward, fill = algorithm)) +
  geom_bar(stat = "identity") +  # Create a bar for each batch size
  labs(title = "Average Reward per Batch Size", 
       x = "Batch Size", 
       y = "Average Reward", 
       fill = "Algorithm") +  # Labels for the plot
...
results <- data.frame(
  ...
  avg_reward = NA,  # Placeholder for average rewards
  algorithm = rep("Epsilon-Greedy (ε = 0.4)", length(batch_sizes))
)

epsilon <- 0.4  # Epsilon value for epsilon-greedy algorithm

# Step 3: Run simulations for the Epsilon-Greedy algorithm and store results
for (i in seq_along(batch_sizes)) {
  batch_size <- batch_sizes[i]
  eps_greedy_total_rewards <- 0  # Reset total rewards for each batch size
  ...
  for (t in seq(1, n_rounds, by = batch_size)) {
    ...
    # Step 4: Calculate average reward for the current batch size
    avg_reward <- eps_greedy_total_rewards / (batch_size * n_users)
    
    # Store the average reward in the results dataframe
    results$avg_reward[i] <- avg_reward
  }
}

# Step 5: Display the results dataframe with calculated average rewards
install.packages("knitr")
library(knitr)
kable(results, caption = "Epsilon-Greedy Simulation Results with Average Rewards")

# Step 6: Create a histogram of the average rewards by batch size
library(ggplot2)

ggplot(data = results, aes(x = factor(batch_size), y = avg_reward, fill = algorithm)) +
  geom_bar(stat = "identity") +  # Create a bar for each batch size
  geom_text(aes(label = round(avg_reward, 2)), vjust = -0.5, size = 3, color = "black") +  # Add labels for average rewards
  labs(title = "Average Reward per Batch Size", 
       x = "Batch Size", 
       y = "Average Reward", 
       fill = "Algorithm") +  # Labels for the plot
