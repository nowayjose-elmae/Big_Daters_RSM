...
```{r, combining item_ids and position}
...
Our initial strategy involved selecting a specific feature and assessing whether any segment within that feature significantly outperformed the others; the details of this analysis are provided in our appendix. However, focusing our analysis on individual segments within each feature proved challenging, as most segments did not show significant differences in clicks upon regression analysis. Consequently, we shifted our approach to combine features, concentrating on enhancing segments already generating clicks. To achieve this, we generated all possible combinations of user features and identified those with click counts greater than zero, focusing our efforts on these segments that were already generating clicks. 
...
```{r, clean my environment 1}
rm(agent_counts, agents_with_clicks, all_combinations, df_sorted_by_agent, num_agents, unique_user_features, unique_agent_count, unique_combinations_count, user_feature_0, user_feature_1, user_feature_2, user_feature_3, df_with_agents, i)
```
...
```{r, data cleaning}
#In order to perform the logit regression for heterogeneity purposes present in the appendix we had to transform our click variable into a factor variable but now we want to make it go back to a numeric variable.
df_filtered_agents$click <- as.numeric(as.character(df_filtered_agents$click))
```

#Calculate the True CTR
...
We picked 4442 rounds per simulation because it matches the number of observations in our dataset, ensuring that each round corresponds to a specific data point. Rounds represent the individual decisions made by the algorithm during each simulation, with each round corresponding to one decision or arm selection based on the Beta distribution sampling. We also ran 10 simulations to ensure that the results are robust and not influenced by randomness, as it averages out the effects of any random fluctuations that might occur in a single run. Given the constraints of our relatively small dataset, which consists of only 4442 observations, we have opted to implement sampling with replacement for our analysis. This allows us to reuse data points across multiple simulations, ensuring that we do not exhaust the limited number of observations too quickly. This approach is particularly important in our scenario where the number of clicks per arm is insufficient for a comprehensive evaluation without replacement (we tried to do a batching analysis without replacement in our appendix with the contextual package). 
...
```
Batch size 1, represented by the red curve, shows a steep increase, indicating rapid learning from each observation, which is beneficial in environments requiring quick adaptation. However, this comes with wide confidence intervals, highlighting a high variability due to the model's sensitivity to noisy and sparse data (4442 observations with only 20 clicks). Batch size 100, shown with the orange curve, strikes a balance by smoothing out fluctuations and offering narrower confidence intervals, making it less reactive to individual observations but still sufficiently responsive. The largest batch size, 4000, depicted by the green curve, provides the most stability, evidenced by the narrowest confidence intervals, but it has as a downside its slow reward accumulation. Despite the higher variability, batch size 1 appears to perform the best in terms of maximizing cumulative rewards.

```{r, clean my environment 2}
rm(batch_results, cumulative_rewards, cumulative_rewards_df, plot_1, plot_2, results, arm_index, arms, batch_chosen_arms, batch_index, batch_rewards, batch_size, batch_sizes, beta_samples, chosen_arm, chosen_arm_index, failures, i, n_arms, n_rounds, n_simulations, reward, round, sim, successes, true_CTR)
```
...
df_filtered_agents %>%
  group_by(combination_index) %>%
  summarise(mean_click = mean(click),  total_click = sum(click)) #there are a lot more failures than success

#Verify Number of Success 
count <- sum(df_filtered_agents$click == 1)
```

Based on the summary of mean_click per arms, we have concluded that there is much more failures than success since the mean_click values are close to zero. These parameters represent our prior belief about the success rate, where $\alpha$ represents the number of observed successes and $\beta$ represents the number of observed failures. A higher initial $\beta$ reflects a bias towards failure. With a slight bias towards failure, 20 different pairs of alphas and betas were chosen to be explored, from (1,10) for alpha and (1,100) for beta.

After comparing the result with various different pairs of alphas and betas, the best pair of alphas and betas were chosen to be (2,70), (1,90) and (3,90) for this dataset. We will use this for the further analysis. The result of the other pairs can be found in the appendix.
...
The results show that the maximum number of rounds across the 10 simulations for each pair of alpha and betas varied between 58 and 95. While most simulations fall within a narrow range, this variation suggests differences in exploration patterns, where some simulations explored more arms or took more actions before reaching the end of the dataset.
...
```
This graph indicates that the alpha-beta pair of alpha=2 and beta=70 achieves the highest average cumulative reward, outperforming other configurations over time. This suggests that it is the most effective combination for maximizing average cumulative rewards. However, the wide confidence intervals associated with this pair also suggest a higher degree of uncertainty in the data, preventing a definitive conclusion about its superiority.

```{r, clean my environment 3}
rm(agent_TS_zozo_20, bandit_Zozo_20, df_history_agg, df_TS_zozo_20, df_TS_zozo_20_max_t, results, results_list, TS, x, y, alphas, betas, count, history_TS_zozo_20, max_obs, n_sim, simulator_20, size_sim, i)
```

#Best Position and Item according to TS
This code implements Thompson Sampling with a batch size of 1, using alpha=2 and beta=70, which were found to be the best-performing parameters from previous analysis. The algorithm is designed to identify the best-performing arm in a multi-armed bandit problem based on cumulative successes over 10 simulations, each with 4442 rounds. For each round, the algorithm samples from a Beta distribution for each arm, selects the arm with the highest sampled value, and simulates a reward based on the true CTR of the chosen arm. Successes and failures for each arm are updated after every round, and the arm with the most cumulative successes across simulations is identified as the best.
...
for (sim in 1:n_simulations) {
  ...
  for (round in 1:n_rounds) {
    ...
    # PREDICT: Sample from the Beta distribution for each arm (with alpha=2, beta=70)
    beta_samples <- rbeta(n_arms, successes + 2, failures + 70)
...
```
```{r}
# Retrieve the row where combination_index is equal to the best arm 
combination_43 <- df_filtered_agents[df_filtered_agents$combination_index == 43, ]

combination_43
```
The best position is 3 and the best item is 11.


```{r, clean my environment 4}
rm(arms, batch_size, best_arm, beta_samples, chosen_arm, chosen_arm_index, failures, n_arms, n_rounds, n_simulations, reward, round, sim, successes, total_successes, true_CTR, combination_43)
...
Regret for a given round is defined as the difference between the maximum possible reward (based on the highest true CTR across all arms) and the actual reward received during that round.
...
# Identify the arm with the maximum CTR
optimal_CTR <- max(true_CTRs$true_CTR)

# Calculate the optimal reward
optimal_reward <- optimal_CTR * size_sim

# Set the seed for reproducibility
set.seed(0)

# Number of simulations and simulation size
n_sim <- 10
size_sim <- 4442

# Define Thompson Sampling policy with parameters
TS <- ThompsonSamplingPolicy$new(alpha = 2, beta = 70)

# Create the bandit setup
bandit_Zozo_20 <- OfflineReplayEvaluatorBandit$new(formula = click ~ combination_index,
                                                   data = df_filtered_agents,
                                                   randomize = FALSE)
...
# Aggregate results to get total rewards
...
df_TS_zozo_20 <- df_TS_zozo_20 %>%
  group_by(t, sim) %>%
  mutate(cumulative_reward = cumsum(total_reward))

# Calculate cumulative regret
df_TS_zozo_20 <- df_TS_zozo_20 %>%
  mutate(cumulative_regret = optimal_reward - cumulative_reward)

# Plotting regret over time
ggplot(df_TS_zozo_20, aes(x = t, y = cumulative_regret, group = sim)) +
  geom_line() +
...
```{r}
set.seed(0)
max_t <- min(df_TS_max_t$max_t) #The lowest value in the max_t for each dataset
...
```
```{r, clean my environment 6}
rm(agent, bandit, click_counts, df_history_agg, df_history_all, df_TS, df_TS_max_t, dfZozo_seg0, dfZozo_seg1, dfZozo_seg2, dfZozo_seg3, feature_0_datasets, logit_model_0, logit_model_1, logit_model_2, logit_model_3, results_list, TS, dataset_names, history, i, max_t, n_sim, name, simulator, size_sim)
```


#Batching with the contextual package 
...
We chose the batch sizes (batch_sizes <- c(1, 5, 10, 20, 50)) to keep max_t sufficiently large so we are able to plot the results. Additionally, we set size_sim to 4442 to match the number of observations in our filtered dataset of agents who clicked on the banner at least once, ensuring the model processes the entire dataset in each simulation.
...
batch_sizes <- c(1, 5, 10, 20, 50)  # Example batch sizes
max_t <- 50  # Maximum time step to filter data
...
for (batch_size in batch_sizes) {
  ...
  df_TS_zozo_20_agg <- df_TS_zozo_20 %>%
    ...
    filter(t <= max_t)  # Correctly placed filter to limit time steps to max_t
...
p1 <- ggplot(df_cumulative_rewards, aes(x = t, y = avg_cumulative_reward, color = as.factor(batch_size))) +
  geom_line(size = 1.2) +
...
p2 <- ggplot(df_cumulative_rewards, aes(x = t, y = avg_cumulative_reward, color = as.factor(batch_size))) +
  geom_line(size = 1.2) +
...
ggplot(data = df_history_agg, aes(x = t, y = avg_cumulative_reward, color = alpha_beta)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
In this graph, the confidence intervals for the different pairs overlap, which indicates that the observed differences between them may not be statistically significant. This overlap is largely a result of the limited sample size in our dataset. When the dataset is small, variability tends to increase, making it more difficult to detect meaningful distinctions between groups. As a result, the confidence intervals widen, and it becomes harder to confidently assert that any observed differences are not due to random chance.
...
#Best combination with the contextual package
...
# Define the Thompson Sampling policy object with best-performing parameters (alpha=2, beta=70)
TS <- ThompsonSamplingPolicy$new(alpha = 2, beta = 70)
...
df_TS_zozo_20 <- history_TS_zozo_20$data %>%
  group_by(choice) %>%
  summarise(total_reward = sum(reward), .groups = 'drop')

# Find the best performing combination_index
best_performance <- df_TS_zozo_20 %>%
  arrange(desc(total_reward)) %>%
  slice(1) %>%
  pull(choice)

# Print the best performing combination_index
...
```{r}
# Retrieve the row where combination_index is equal to the best arm 
combination_26 <- df_filtered_agents[df_filtered_agents$combination_index == 26, ]

combination_26
```
The best position is 2 and the best item is 14.

```{r}
rm(batch_sizes, best_combination_indices, best_performance, history_TS_zozo_20, n_sim, simulator, size_sim, combination_26, agent_TS_zozo_20, bandit_Zozo_20, TS)
```
